...
export default class CartController extends BaseController {
    ...
    addProductToCart = async (req, res) => {
        ...
        try {
            if (!exist) {
                ...
                await this.cartManager.increaseProductQuantity(cid, pid);
    ...
    decreaseProductQuantity = async (req, res) => {
        ...
        try {
            await this.cartManager.decreaseProductQuantity(cid, pid);
            res.sendSuccess()
        } catch (error) {
            res.sendInternalError(error)
        }
    ...
    removeProductFromCart = async (req, res) => {
        const { cid, pid } = req.params;
        const cart = await this.cartManager.getBy({ _id: cid });
        if (!cart) return res.sendNotFound("Cart Not Found.");
        const product = cart.products.filter(e => e.product._id.toString() == pid);
        console.log(product)
        if (product.length === 0) return res.sendNotFound("Product Not Found.");
        try {
            const result = await this.cartManager.removeProductFromCart(cid, pid);
